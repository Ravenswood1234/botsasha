...
class Helps(commands.Cog):
	...
	async def funny(self, ctx ):
		...
		emb.add_field( name = '`{}rpc(камень, ножницы, бумага)`'.format( PREFIX ), value = '**Игра в камень, ножницы, бумага!**')
	...
	async def kindcoin(self, ctx):
		...
		emb.add_field( name = f'`{PREFIX}take_kindcoin`', value = '**Забрать kindcoin у пользователя (для адм)**')
		await ctx.send(embed = emb)
