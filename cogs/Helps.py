...
class Helps(commands.Cog):
	...
	async def help(self, ctx):
		emb = discord.Embed( title = f'**Выбирите категорию! Мой префикс: {PREFIX}**', description = f'Категории: ``moderation | funny | other | economy | lvl``.    Пример: ``{PREFIX}help_funny``!', colour = discord.Color.red())
	...
	async def help_moderation(self, ctx ):
		...
		emb.add_field( name = '`{}ban (@пользовател) (время) (s/m/h/d)`'.format( PREFIX ), value = '**Забанить пользователя (для адм)**')
	...
	async def help_lvl(self, ctx):
		...
		emb.add_field( name = '`{}lvl`'.format( PREFIX ), value = '**Узнать какой у вас уровень! Если указать пользователя то вы сможите узнать его лвл!**')
