...
class Moder(commands.Cog):
    ...
    async def mute(self, ctx, member: discord.Member = None, duration: int = None, tm: str = None, *, arg = None):
        ...
        if member is None:
            ...
            emb.add_field(name="Замутил:", value=f'{ctx.author.mention} __**замутил**__: {member.mention} __**на  {duration} {tm}.**__')
    ...
    @commands.command()
    @commands.has_permissions( administrator = True )
    async def tempban(self, ctx, member: discord.Member = None, reason = None, amount: int = None, *, tm: str = None ):
        if member is None:
            ...
            await ctx.send('Укажите сколько будет действовать мут s = секунд, m = минут, h = час, d = день')
        else:
            ...
            if tm == 's':
                ...
                await asyncio.sleep(duration * 86400)
            emb1 = discord.Embed(title = '**Размут!**', description = f'**У {member.mention}, прошло время мута!**', colour = discord.Color.green())
