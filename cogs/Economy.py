...
class Economy(commands.Cog):
	...
	async def __balance(self, ctx, member: discord.Member = None):
		if member is None:
			if cursor.execute("SELECT adminstaff FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0] == 1:
				...
				emb.add_field( name = 'Наличные:', value = f'{cursor.execute("SELECT cash FROM users WHERE id = {} AND guild_id = {}".format(ctx.author.id, ctx.guild.id)).fetchone()[0]} :leaves:' )
				emb.add_field( name = 'Банк:', value = f'{cursor.execute("SELECT bank FROM users WHERE id = {} AND guild_id = {}".format(ctx.author.id, ctx.guild.id)).fetchone()[0]} :leaves:')
				emb.add_field( name = 'Kindcoins:', value = f'{cursor.execute("SELECT kindcoin FROM users WHERE id = {} AND guild_id = {}".format(ctx.author.id, ctx.guild.id)).fetchone()[0]} :moneybag:')
			...
			if cursor.execute("SELECT adminstaff FROM users WHERE id = {}".format(member.id)).fetchone()[0] == 1:
				...
				emb.add_field( name = 'Наличные:', value = f'{cursor.execute("SELECT cash FROM users WHERE id = {} AND guild_id = {}".format(member.id, ctx.guild.id)).fetchone()[0]} :leaves:' )
				emb.add_field( name = 'Банк:', value = f'{cursor.execute("SELECT bank FROM users WHERE id = {} AND guild_id = {}".format(member.id, ctx.guild.id)).fetchone()[0]} :leaves:')
				emb.add_field( name = 'Kindcoins:', value = f'{cursor.execute("SELECT kindcoin FROM users WHERE id = {} AND guild_id = {}".format(member.id, ctx.guild.id)).fetchone()[0]} :moneybag:')
	...
	async def put(self, ctx, amount: int = None):
		if amount is None:
			...
			cursor.execute("UPDATE users SET cash = cash - {} WHERE id = {} AND guild_id = {}".format(amount, ctx.author.id, ctx.guild.id))
			connection.commit()
			cursor.execute("UPDATE users SET bank = bank + {} WHERE id = {} AND guild_id = {}".format(amount, ctx.author.id, ctx.guild.id))
			connection.commit()
	...
	async def add_money(self, ctx, member: discord.Member = None, amount: int = None):
		...
		if member is None:
			...
			if amount is None:
				...
				cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(amount, member.id, ctx.guild.id))
				connection.commit()
	...
	async def take_money(self, ctx, member: discord.Member = None, amount = None):
		...
		if member is None:
			...
			if amount is None:
				...
				cursor.execute("UPDATE users SET cash = cash - {} WHERE id = {} AND guild_id = {}".format(int(amount), member.id, ctx.guild.id))
				connection.commit()
	...
	async def br(self, ctx, amount: int = None):
		...
		if amount is None:
			...
			if amount < 0:
				...
				self.br.reset_cooldown(ctx)
			elif cursor.execute("SELECT cash FROM users WHERE id = {} AND guild_id = {}".format(ctx.author.id, ctx.guild.id)).fetchone()[0] < amount:
				...
				connection.commit()
				if number > 60:
					cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(int(amount *2), ctx.author.id, ctx.guild.id))
					connection.commit()
	...
	async def work(self, ctx, member: discord.Member = None):
		...
		if a == 1:
			...
			await ctx.send(embed = emb)
			cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(2000, ctx.author.id, ctx.guild.id))
			connection.commit()
		else:
			emb = discord.Embed(title = '**Работа!**', description =  f'**Пользователь: {ctx.author.name}, сходил на работу и получил 2000:leaves:!(Следующий рабочий день через 2 часа!)**')
			await ctx.send(embed = emb)
