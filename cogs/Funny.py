...
class Funny(commands.Cog):
	...
	async def duel(self, ctx, member: discord.Member = None, amount: int = None ):
		...
		if ctx.author == member:
			...
			self.duel.reset_cooldown(ctx)
		elif amount > cursor.execute("SELECT cash FROM users WHERE id = {} AND guild_id = {}".format(member.id, ctx.guild.id)).fetchone()[0]:
	...
	async def run(self, ctx, member: discord.Member = None):
		...
		if ctx.author == member:
			...
			if a == 1:
				...
				cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(1000, ctx.author.id, ctx.guild.id))
				connection.commit()
			else:
				await ctx.send(embed = embbb)
				cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(1000, member.id, ctx.guild.id))
				connection.commit()
	...
	async def luck(self, ctx):
		...
		if number > number2:
			...
			cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {} AND guild_id = {}".format(50, ctx.author.id, ctx.guild.id))
			connection.commit()
			await ctx.send(embed = emb)
		else:
